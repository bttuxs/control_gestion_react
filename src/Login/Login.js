...
class Login extends React.Component{
    ...
    async getUsers(e){
        ...
        try{
            ...
            this.context.setGlobalSpinner(false)
            this.context.setGlobalAlert({show: true, message: 'Error en la comunicacion', type: 'error'})
        }
        return false;        
    }
                ...
                <div>
                    ...
                    <form className="mt-8 space-y-6" action="#" method="POST">
                        ...
                        <div className="rounded-md shadow-sm -space-y-px">
                            ...
                            <input id="email-address" name="user" type="text" value={this.state.form.user || ''} onChange={this.handleInputChange} autoComplete="off" required className="appearance-none rounded-none relative block w-full px-3 py-2 border border-gray-300 placeholder-gray-500 text-gray-900 rounded-t-md focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 focus:z-10 sm:text-sm" placeholder="Usuario" />
                            </div>
                            <div>
                            <label htmlFor="password" className="sr-only">Password</label>
                            <input id="password" name="pass" type="password" value={this.state.form.pass || ''} onChange={this.handleInputChange} autoComplete="off" required className="appearance-none rounded-none relative block w-full px-3 py-2 border border-gray-300 placeholder-gray-500 text-gray-900 rounded-b-md focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 focus:z-10 sm:text-sm" placeholder="Password" />
                            </div>
                        </div>
                        <div>
